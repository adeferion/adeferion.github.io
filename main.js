...
function dumpgbotF(replay) {
    const buffer = new ArrayBuffer(12 + replay.actions.length * 8);
    const view = new DataView(buffer);
    strToBuf('gBot').forEach((n, i) => view.setUint8(i, n));
        ...
        if (from === 'txt') {
            ...
            switch (from) {
                ...
                case 'zbot':
                    ...
                    break;
                case 'gbot':
                    replay = parsegBot(await files[0].text());
                    break;
                case 'ddhor':
